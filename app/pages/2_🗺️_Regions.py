"""
Regional Analysis Streamlit page featuring choropleth map visualization and cross-regional comparisons.
"""
...
from api_client import get_eurostat_boundaries
...
def load_regions_data() -> pd.DataFrame:
    """Fetches tourism records from database with caching for regional analysis.

    Returns:
        pd.DataFrame: Cleaned regional dataset.
    """
    ...
    try:
        ...
        logger.warning(f"Database fetch warning in regions page: {e}")

    return load_fallback_df()


df: pd.DataFrame = load_regions_data()

if df.empty:
    render_error_banner(message="No regional data available.", lang=lang)
    st.stop()
...
with col_filter:
    ...
    selected_year = st.selectbox(
        t("select_year", lang), [all_years_label] + years, key="reg_select_year"
    )
...
# Aggregate by region
region_data = (
    filtered_df.groupby(["geo", "geo_label"])[["arrivals", "receipts", "overnights"]]
    .sum()
    .reset_index()
)
...
# Geographic Map Section with cached boundary fetching
@st.cache_data(ttl=86400)
def fetch_cached_boundaries() -> Dict[str, Any]:
    """Fetches Eurostat GeoJSON boundaries cached for 24 hours."""
    return get_eurostat_boundaries()


geojson = fetch_cached_boundaries()
...
with tab2:
    ...
    fig_bar_overnights.update_layout(
...
with tab3:
    ...
    other_receipts = (
        region_data.sort_values(by="receipts", ascending=False)
        .iloc[5:]["receipts"]
        .sum()
    )
...
with tab4:
    ...
    with comp_col1:
        reg_a = st.selectbox(
            t("select_region_a", lang), all_regions, index=0, key="reg_comp_a"
        )
    with comp_col2:
        default_idx = 1 if len(all_regions) > 1 else 0
        reg_b = st.selectbox(
            t("select_region_b", lang), all_regions, index=default_idx, key="reg_comp_b"
        )

    df_comp = (
        df[df["geo_label"].isin([reg_a, reg_b])]
        .groupby(["geo_label", "year"])[["arrivals", "overnights", "receipts"]]
        .sum()
        .reset_index()
    )
    ...
    c_tab1, c_tab2, c_tab3 = st.tabs(
        [t("tab_receipts", lang), t("tab_alos", lang), t("tab_yield", lang)]
    )
