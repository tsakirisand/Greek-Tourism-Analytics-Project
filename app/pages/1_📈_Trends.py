"""
Trends Analysis Streamlit page for tracking multi-year tourism performance metrics.
"""
...
def load_trends_data() -> pd.DataFrame:
    """Fetches full tourism dataset for multi-year trend analysis with caching.
    ...
    try:
        ...
        logger.warning(f"Trends page data fetch warning: {e}")

    return load_fallback_df()


df: pd.DataFrame = load_trends_data()

if df.empty:
    render_error_banner(message="No data found for trend analysis.", lang=lang)
    st.stop()
...
yearly_data = (
    df.groupby("year")[["arrivals", "overnights", "receipts", "occupancy"]]
    .sum()
    .reset_index()
)

# Compute Advanced Industry KPIs
...
st.divider()
st.subheader("Data Table / Πίνακας Δεδομένων")
display_data = (
    yearly_data.drop(columns=["occupancy"]) if not has_occupancy else yearly_data
)
st.dataframe(
    display_data.rename(
        columns={
